...
df <- ldply(datalist.orig, poolallstuff)
...
# qplot(x = factor(time), y = value, data = df[df$exp.id=="140509_IVIS",], geom = "boxplot")
...
qplot(x = factor(exp.id), y = value, data=df, geom = "boxplot") + facet_grid(Instrument~time, scales = "free_y")

# lets work on with 72 hour datapoint ----
threedays <- df[df$time==72,]

# ok, let's check if diferent experiments values differ
fits <- dlply(threedays, c("Instrument"), function(x) lm(value~factor(exp.id), data = x))
lapply(fits, anova)

# plot data at 72 hour timepoint
qplot(x = factor(exp.id), y = value, data=threedays, geom = "boxplot") + facet_grid(Instrument~time, scales = "free_y")

# lets detrend data then -----
threedays <- ddply(threedays, "Instrument", transform, value_db = detrend(value, exp.id))

# was detrending effective
fits <- dlply(threedays, c("Instrument"), function(x) lm(value_db ~ factor(exp.id), data = x))
lapply(fits, anova) # yep seems so ...
qplot(x = factor(exp.id), y = value_db, data=threedays, geom = "boxplot") + facet_grid(Instrument~time, scales = "free_y")
# 

#threedays <- ddply(threedays, "exp.id", transform, value = scale(value, center = FALSE))
...
q <- ggplot(summary, aes(x = log10(doses), y = Mean, colour = treatment)) +
  ...
  geom_errorbar(aes(ymin = Mean - SE, ymax = Mean + SE), width=0.2) + ylab(expression(Mean %+-% SE))
...
ggsave(file=paste0("graphs/cellgrowth_exp_summary_", Sys.Date(),".pdf"), g)
...
summary2 <- ddply(threedays, c("doses","treatment", "Instrument"), summarize,
                  Mean = mean(normvalue),
                  SD = sd(normvalue),
                  N = length(normvalue),
                  SE = SD/sqrt(N))
...
ggplot(summary2, aes(x = log10(doses), y = Mean, colour = treatment)) +
  geom_point(size = 3) + facet_wrap(~Instrument, scales = "free_y") + geom_line(size = 1) +
  geom_errorbar(aes(ymin = Mean - SE, ymax = Mean + SE), width=0.2) + 
  ylab(expression(Mean %+-% SE))
ggsave(paste0("graphs/cellgrowth_norm", Sys.Date(),".pdf"))
